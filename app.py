...
def removing_friend_custom():
    """URL: /remove_friend?uid={uid}&password={password}&friend_uid={target_uid}"""
    uid = request.args.get('uid')
    password = request.args.get('password')
    friend_uid = request.args.get('friend_uid')
    server_name = request.args.get('server_name', 'IND')
    ...
    author_uid = decode_author_uid(token)
    if not author_uid:
        return jsonify({"status": "failed", "message": "Failed to decode author UID"}), 400

    try:
        result = remove_friend_with_retry(author_uid, friend_uid, token, server_name)
        return jsonify(result)
    except Exception as e:
        return jsonify({"status": "failed", "message": f"Remove friend error: {str(e)}"})
...
def home_route():
    ...
    response["region"] = "IND - INDIA"
    response["success"] = True
    response["message"] = "ALL IN ONE ADD/REMOVE API"
    response["api_dev"] = "@FFDEVILX"
    response["endpoints"] = OrderedDict({
        "/adding_friend": {
            "method": "GET",
            "description": "Send a friend request to another player",
            "parameters": {
                "uid": "Your account UID (required)",
                "password": "Your account password (required)",
                "friend_uid": "UID of friend to add (required)"
            },
            "example": "/adding_friend?uid=123456&password=abcd1234&friend_uid=987654"
        },
        "/remove_friend": {
            "method": "GET",
            ...
            "parameters": {
                "uid": "Your account UID (required)",
                "password": "Your account password (required)",
                "friend_uid": "UID of friend to remove (required)"
            },
            "example": "/remove_friend?uid=123456&password=abcd1234&friend_uid=987654"
        },
        "/player_info": {
            "method": "GET",
            "description": "Fetch detailed player information",
            "parameters": {
                "uid": "Your account UID (required)",
                "password": "Your account password (required)",
                "friend_uid": "UID of player to get info for (required)"
            },
            "example": "/player_info?uid=123456&password=abcd1234&friend_uid=987654"
        },
        "/token": {
            "method": "GET",
            "description": "Generate a JWT token for your account using UID and password",
            "parameters": {
                "uid": "Your account UID (required)",
                "password": "Your account password (required)"
            },
            "example": "/token?uid=123456&password=abcd1234"
        },
        "/health": {
            "method": "GET",
